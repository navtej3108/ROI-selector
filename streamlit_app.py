...
def main():
    ...
    if uploaded_file is not None:
        ...
        img_rgb = load_image(uploaded_file.read())

        # Display the image with rectangles
        st.image(img_rgb, channels="RGB", use_column_width=True)

        # Initialize session state for storing rectangles if not already done
        ...
        with st.form(key='roi_form'):
            ...
            add_roi = st.form_submit_button("Add ROI")

            # Add the rectangle to the session state
            if add_roi:
        ...
        for rect in st.session_state['rects']:
            cv2.rectangle(img_with_rects, (int(rect[0]), int(rect[1])), 
        ...
        st.image(img_with_rects, channels="RGB", use_column_width=True)

        # Save ROI data to an Excel file
        if st.button("Save ROIs to Excel"):
            roi_data = [
                {"ROI": f"ROI {i+1}", "Name": rect[4], "Code": rect[5], 
